...
public class GeoEngine extends GeoData
{
	...
	@Override
	public boolean canSeeTarget(L2Object cha, Location target)
	{
	...
	private static void nInitGeodata()
	{
		...
		if (!file.exists())
		{
			_log.warn("File not found " + file.getAbsolutePath() + " Geodata is disabled.");
			Config.GEODATA = 0;
			return;
		}
		try (LineNumberReader lnr = new LineNumberReader(new BufferedReader(new FileReader(file))))
		{
			_log.info("GeoEngine loading Geodata from " + file.getAbsolutePath() + " ...");
			String line;
	...
	public static boolean loadGeodataFile(byte rx, byte ry)
	{
		...
		{
			_log.warn("Failed to Load GeoFile: invalid region " + rx + "," + ry + Config.EOL);
			return false;
		}
		String fname = rx + "_" + ry + ".l2j";
		short regionoffset = (short) ((rx << 5) + ry);
		_log.info("GeoEngine loading: " + fname + " -> region offset: " + regionoffset + " X: " + rx + " Y: " + ry);
			...
			FileChannel roChannel = raf.getChannel())
		{
			...
			_log.info("GeoEngine: Max Layers: " + flor + " Size: " + size + " Loaded: " + index);
		}
		catch (Exception e)
		{
	...
	private static short nGetSpawnHeight(int geox, int geoy, int zmin, int zmax, L2Spawn spawn)
	{
		...
		// multilevel
		{
			...
			if ((temph > (zmax + 200)) || (temph < (zmin - 200)))
			{
				if (Config.DEBUG)
				{
					_log.warn("SpawnHeight Error - Couldnt find correct layer to spawn NPC - GeoData or Spawnlist Bug!: zmin: " + zmin + " zmax: " + zmax + " value: " + temph + " Spawn: " + spawn + " at: " + geox + " : " + geoy);
				}
				return (short) zmin;
			}
		}
		if ((temph > (zmax + 1000)) || (temph < (zmin - 1000)))
		{
			if (Config.DEBUG)
			{
				_log.warn("SpawnHeight Error - Spawnlist z value is wrong or GeoData error: zmin: " + zmin + " zmax: " + zmax + " value: " + temph + " Spawn: " + spawn + " at: " + geox + " : " + geoy);
			}
			return (short) zmin;
		}
		return temph;
