...
public class GeoData implements IGeoDriver
{
	...
	private static final int _MAX_SEE_OVER_HEIGHT = 50;
	...
	public boolean canSeeTarget(int x, int y, int z, int tx, int ty, int tz)
	{
		...
		double fullDist = Util.calculateDistance(geoX, geoY, tGeoX, tGeoY);
		if (!(fullDist > 0.0))
		{
			return z == tz;
		}
		...
		while (pointIter.next())
		{
			...
			{
				double percentageDist = Util.calculateDistance(geoX, geoY, curX, curY) / fullDist;
				int curZ = (int) (z + (fullZDiff * percentageDist));
				int maxHeight = curZ + _MAX_SEE_OVER_HEIGHT;
