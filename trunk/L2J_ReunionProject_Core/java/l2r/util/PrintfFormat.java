 ...
 * </p>
 * <p>
 * The results are undefined if there are insufficient arguments for the format. Usually an unchecked exception will be thrown. If the format is exhausted while arguments remain, the excess arguments are evaluated but are otherwise ignored. In format strings containing the % form of conversion
 * specifications, each argument in the argument list is used exactly once.
 * </p>
 * <p>
 * Conversions can be applied to the <code>n</code>th argument after the format in the argument list, rather than to the next unused argument. In this case, the conversion characer % is replaced by the sequence %<code>n</code>$, where <code>n</code> is a decimal integer giving the position of the
 ...
 * </p>
 * <p>
 * An optional minimum field width. If the converted value has fewer characters than the field width, it will be padded with spaces by default on the left; t will be padded on the right, if the left- adjustment flag (-), described below, is given to the field width. The field width takes the form of
 * a decimal integer. If the conversion character is s, the field width is the the minimum number of characters to be printed.
 * </p>
 * <p>
 * An optional precision that gives the minumum number of digits to appear for the d, i, o, x or X conversions (the field is padded with leading zeros); the number of digits to appear after the radix character for the e, E, and f conversions, the maximum number of significant digits for the g and G
 * conversions; or the maximum number of characters to be written from a string is s and S conversions. The precision takes the form of an optional decimal digit string, where a null digit string is treated as 0. If a precision appears with a c conversion character the precision is ignored.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * A field width or precision may be indicated by an asterisk (*) instead of a digit string. In this case, an integer argument supplised the field width precision. The argument that is actually converted is not fetched until the conversion letter is seen, so the the arguments specifying field width
 * or precision must appear before the argument (if any) to be converted. If the precision argument is negative, it will be changed to zero. A negative field width argument is taken as a - flag, followed by a positive field width.
 * </p>
 * <p>
 * In format strings containing the %<code>n</code>$ form of a conversion specification, a field width or precision may be indicated by the sequence *<code>m</code>$, where m is a decimal integer giving the position in the argument list (after the format argument) of an integer argument containing
 * the field width or precision.
 * </p>
 * <p>
 * The format can contain either numbered argument specifications (that is, %<code>n</code>$ and *<code>m</code>$), or unnumbered argument specifications (that is % and *), but normally not both. The only exception to this is that %% can be mixed with the %<code>n</code>$ form. The results of mixing
 * numbered and unnumbered argument specifications in a format string are undefined.
 * </p>
 ...
 * <dl>
 * <dt>'
 * <dd>integer portion of the result of a decimal conversion (%i, %d, %f, %g, or %G) will be formatted with thousands' grouping characters. For other conversions the flag is ignored. The non-monetary grouping character is used.
 * <dt>-
 ...
 * <dt>+
 * <dd>result of a signed conversion always begins with a sign (+ or -). (It will begin with a sign only when a negative value is converted if this flag is not specified.)
 * <dt>&lt;space&gt;
 * <dd>If the first character of a signed conversion is not a sign, a space character will be placed before the result. This means that if the space character and + flags both appear, the space flag will be ignored.
 * <dt>#
 * <dd>value is to be converted to an alternative form. For c, d, i, and s conversions, the flag has no effect. For o conversion, it increases the precision to force the first digit of the result to be a zero. For x or X conversion, a non-zero result has 0x or 0X prefixed to it, respectively. For e,
 * E, f, g, and G conversions, the result always contains a radix character, even if no digits follow the radix character (normally, a decimal point appears in the result of these conversions only if a digit follows it). For g and G conversions, trailing zeros will not be removed from the result as
 * they normally are.
 * <dt>0
 * <dd>d, i, o, x, X, e, E, f, g, and G conversions, leading zeros (following any indication of sign or base) are used to pad to the field width; no space padding is performed. If the 0 and - flags both appear, the 0 flag is ignored. For d, i, o, x, and X conversions, if a precision is specified,
 ...
 * <dl>
 * <dt>d,i
 * <dd>The int argument is converted to a signed decimal in the style [-]dddd. The precision specifies the minimum number of digits to appear; if the value being converted can be represented in fewer digits, it will be expanded with leading zeros. The default precision is 1. The result of converting
 * 0 with an explicit precision of 0 is no characters.
 * <dt>o
 * <dd>The int argument is converted to unsigned octal format in the style ddddd. The precision specifies the minimum number of digits to appear; if the value being converted can be represented in fewer digits, it will be expanded with leading zeros. The default precision is 1. The result of
 * converting 0 with an explicit precision of 0 is no characters.
 * <dt>x
 * <dd>The int argument is converted to unsigned hexadecimal format in the style dddd; the letters abcdef are used. The precision specifies the minimum numberof digits to appear; if the value being converted can be represented in fewer digits, it will be expanded with leading zeros. The default
 ...
 * <dt>f
 * <dd>The floating point number argument is written in decimal notation in the style [-]ddd.ddd, where the number of digits after the radix character (shown here as a decimal point) is equal to the precision specification. A Locale is used to determine the radix character to use in this format. If
 * the precision is omitted from the argument, six digits are written after the radix character; if the precision is explicitly 0 and the # flag is not specified, no radix character appears. If a radix character appears, at least 1 digit appears before it. The value is rounded to the appropriate
 * number of digits.
 * <dt>e,E
 * <dd>The floating point number argument is written in the style [-]d.ddde{+-}dd (the symbols {+-} indicate either a plus or minus sign), where there is one digit before the radix character (shown here as a decimal point) and the number of digits after it is equal to the precision. A Locale is used
 * to determine the radix character to use in this format. When the precision is missing, six digits are written after the radix character; if the precision is 0 and the # flag is not specified, no radix character appears. The E conversion will produce a number with E instead of e introducing the
 * exponent. The exponent always contains at least two digits. However, if the value to be written requires an exponent greater than two digits, additional exponent digits are written as necessary. The value is rounded to the appropriate number of digits.
 * <dt>g,G
 * <dd>The floating point number argument is written in style f or e (or in sytle E in the case of a G conversion character), with the precision specifying the number of significant digits. If the precision is zero, it is taken as one. The style used depends on the value converted: style e (or E)
 * will be used only if the exponent resulting from the conversion is less than -4 or greater than or equal to the precision. Trailing zeros are removed from the result. A radix character appears only if it is followed by a digit.
 * <dt>c,C
 * <dd>The integer argument is converted to a char and the result is written.
 * <dt>s,S
 * <dd>The argument is taken to be a string and bytes from the string are written until the end of the string or the number of bytes indicated by the precision specification of the argument is reached. If the precision is omitted from the argument, it is taken to be infinite, so all characters up to
 * the end of the string are written.
 * <dt>%
...
public class PrintfFormat
{
	...
	private class ConversionSpecification
	{
		...
		/**
		 * For f format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. '+' character means that the conversion will always begin with a sign (+ or -). The blank flag character means that a non-negative input
		 * will be preceded with a blank. If both a '+' and a ' ' are specified, the blank flag is ignored. The '0' flag character implies that padding to the field width will be done with zeros instead of blanks. The field width is treated as the minimum number of characters to be printed. The
		 * default is to add no padding. Padding is with blanks by default. The precision, if set, is the number of digits to appear after the radix character. Padding is with trailing 0s.
		 * @param x
		 * @return
		 */
		...
		/**
		 * An intermediate routine on the way to creating an f format String. The method decides whether the input double value is an infinity, not-a-number, or a finite double and formats each type of input appropriately.
		...
		/**
		 * For e format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. '+' character means that the conversion will always begin with a sign (+ or -). The blank flag character means that a non-negative input
		 * will be preceded with a blank. If both a '+' and a ' ' are specified, the blank flag is ignored. The '0' flag character implies that padding to the field width will be done with zeros instead of blanks. The field width is treated as the minimum number of characters to be printed. The
		 * default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear after the radix character. Padding is with trailing 0s. The behavior is like printf. One (hopefully the only) exception is that the minimum number of exponent
		 * digits is 3 instead of 2 for e and E formats when the optional L is used before the e, E, g, or G conversion character. The optional L does not imply conversion to a long long double.
		 * @param x
		 * @param eChar
		 * @return
		 */
		...
		/**
		 * Start the symbolic carry process. The process is not quite finished because the symbolic carry may change the length of the string and change the exponent (in e format).
		 * @param ca
		...
		/**
		 * Format method for the g conversion character. For g format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. '+' character means that the conversion will always begin with a sign (+ or -). The blank
		 * flag character means that a non-negative input will be preceded with a blank. If both a '+' and a ' ' are specified, the blank flag is ignored. The '0' flag character implies that padding to the field width will be done with zeros instead of blanks. The field width is treated as the
		 * minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear after the radix character. Padding is with trailing 0s.
		 * @param x the double to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the d conversion specifer and short argument. For d format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. A '+' character means that the conversion will always begin with a sign
		 * (+ or -). The blank flag character means that a non-negative input will be preceded with a blank. If both a '+' and a ' ' are specified, the blank flag is ignored. The '0' flag character implies that padding to the field width will be done with zeros instead of blanks. The field width is
		 * treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		...
		/**
		 * Format method for the d conversion character and long argument. For d format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. A '+' character means that the conversion will always begin with a sign
		 * (+ or -). The blank flag character means that a non-negative input will be preceded with a blank. If both a '+' and a ' ' are specified, the blank flag is ignored. The '0' flag character implies that padding to the field width will be done with zeros instead of blanks. The field width is
		 * treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the long to format.
		 * @return the formatted String.
		...
		/**
		 * Format method for the d conversion character and int argument. For d format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. A '+' character means that the conversion will always begin with a sign (+
		 * or -). The blank flag character means that a non-negative input will be preceded with a blank. If both a '+' and a ' ' are specified, the blank flag is ignored. The '0' flag character implies that padding to the field width will be done with zeros instead of blanks. The field width is
		 * treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the int to format.
		 * @return the formatted String.
		...
		/**
		 * Format method for the x conversion character and short argument. For x format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The '#' flag character means to lead with '0x'. The field width is
		 * treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the short to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the x conversion character and long argument. For x format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The '#' flag character means to lead with '0x'. The field width is
		 * treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the long to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the x conversion character and int argument. For x format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The '#' flag character means to lead with '0x'. The field width is treated
		 * as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the int to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the o conversion character and short argument. For o format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The '#' flag character means that the output begins with a leading 0 and
		 * the precision is increased by 1. The field width is treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the short to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the o conversion character and long argument. For o format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The '#' flag character means that the output begins with a leading 0 and
		 * the precision is increased by 1. The field width is treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the long to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the o conversion character and int argument. For o format, the flag character '-', means that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The '#' flag character means that the output begins with a leading 0 and
		 * the precision is increased by 1. The field width is treated as the minimum number of characters to be printed. The default is to add no padding. Padding is with blanks by default. The precision, if set, is the minimum number of digits to appear. Padding is with leading 0s.
		 * @param x the int to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * Format method for the c conversion character and char argument. The only flag character that affects c format is the '-', meaning that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The field width is treated as the minimum number of
		 * characters to be printed. Padding is with blanks by default. The default width is 1. The precision, if set, is ignored.
		...
		/**
		 * Format method for the s conversion character and String argument. The only flag character that affects s format is the '-', meaning that the output should be left justified within the field. The default is to pad with blanks on the lel2ft. The field width is treated as the minimum number
		 * of characters to be printed. The default is the smaller of the number of characters in the the input and the precision. Padding is with blanks by default. The precision, if set, specifies the maximum number of characters to be printed from the string. A null digit string is treated as a
		 * 0. The default is not to set a maximum number of characters to be printed.
		 * @param x the String to format.
		 * @return the formatted String.
		 */
		...
		/**
		 * For an o conversion, increase the precision to force the first digit of the result to be a zero. For x (or X) conversions, a non-zero result will have 0x (or 0X) prepended to it. For e, E, f, g, or G conversions, the result will always contain a radix character, even if no digits follow
		 * the point. For g and G conversions, trailing zeros will not be removed from the result.
		 */
		...
		/**
		 * The minimum number of digits to appear for the d, i, o, u, x, or X conversions. The number of digits to appear after the radix character for the e, E, and f conversions. The maximum number of significant digits for the g and G conversions. The maximum number of bytes to be printed from a
