...
public final class CastleManorManager extends DocumentParser implements IStorable
{
	...
	private final void loadDb()
	{
			...
			for (Castle castle : CastleManager.getInstance().getCastles())
			{
				...
				try (ResultSet rs = stProduction.executeQuery())
				{
					while (rs.next())
					{
						...
						{
							_log.warn(getClass().getSimpleName() + ": Unknown seed id: " + seedId + "!");
						}
					}
				}
				...
				try (ResultSet rs = stProcure.executeQuery())
				{
					...
					while (rs.next())
					{
						...
						{
							_log.warn(getClass().getSimpleName() + ": Unknown crop id: " + cropId + "!");
						}
					}
				}
		...
		{
			_log.warn(getClass().getSimpleName() + ": Unable to load manor data! " + e.getMessage());
		}
	}
	...
	public final void setNextSeedProduction(List<SeedProduction> list, int castleId)
	{
		...
		if (Config.ALT_MANOR_SAVE_ALL_ACTIONS)
		{
			...
			{
				_log.error(getClass().getSimpleName() + ": Unable to store manor data! " + e.getMessage());
			}
		}
	}
