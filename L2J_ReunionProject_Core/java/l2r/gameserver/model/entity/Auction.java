...
public class Auction
{
	...
	private boolean takeItem(L2PcInstance bidder, long quantity)
	{
		if (CustomServerConfigs.ALTERNATE_PAYMODE_CLANHALLS)
		{
			if ((bidder.getClan() != null) && (bidder.getClan().getWarehouse().getFAdena() >= quantity))
			{
				bidder.getClan().getWarehouse().destroyItemByItemId("Buy", FADENA_ID, quantity, bidder, bidder);
				return true;
			}
		}
		else
