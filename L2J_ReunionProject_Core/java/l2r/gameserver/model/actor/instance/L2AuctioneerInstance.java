		...
		else if (condition == COND_REGULAR)
		{
			...
			else if (actualCommand.equalsIgnoreCase("bid1"))
			{
				...
				try
				{
					...
					{
						html.replace("%PLEDGE_ADENA%", String.valueOf(player.getClan().getWarehouse().getFAdena()));
					}
					else
					{
			...
			else if (actualCommand.equalsIgnoreCase("sale"))
			{
				...
				if (CustomServerConfigs.ALTERNATE_PAYMODE_CLANHALLS)
				{
					html.replace("%AGIT_PLEDGE_ADENA%", String.valueOf(player.getClan().getWarehouse().getFAdena()));
				}
				else
				{
